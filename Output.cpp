...
/**************************************************************************************/
#if defined(PROMINI)
  uint8_t DRIVER_PIN[5] = {4,3,7,6,8};  //{5,6,7,8,4};   //STEP1 (PORTD 5), STEP2 (PORTD 6), DIR1 (PORTD 7), DIR2 (PORTB 0), ENABLE
#endif
#if defined(PROMICRO)
#endif
#if defined(MEGA)
...
void setMotorSpeed(uint8_t motorNum, int16_t speed) {
  ...
  if ((actualMotorSpeed[0] == 0) && (actualMotorSpeed[1] == 0)) {
    digitalWrite(8,HIGH);//(4,HIGH);   // Disable motors
  } else {
    digitalWrite(8,LOW);//(4,LOW);   // Enable motors
  }  
}
...
ISR(TIMER1_COMPA_vect) // DRIVER_PIN[5] = {5,6,7,8,4};   //STEP1 (PORTD 5), STEP2 (PORTD 6), DIR1 (PORTD 7), DIR2 (PORTB 0), ENABLE
{
  ...
  if (periodsCounter[0] >= subPeriod[0][subPeriodIndex[0]]) {
    ...
    if (subPeriod[0][0] != ZERO_SPEED) {
      if (actualMotorDir[0]) {
        SET(PORTD,7);//(PORTD,7);  // DIR Motor 1
      } else {
        CLR(PORTD,7);//(PORTD,7);
      }  
      ...
      SET(PORTD,4);//(PORTD,5); // STEP Motor 1
      delayMicroseconds(1);
      CLR(PORTD,4);//(PORTD,5);
    }
  }
  ...
  if (periodsCounter[1] >= subPeriod[1][subPeriodIndex[1]]) {
    ...
    if (subPeriod[1][0] != ZERO_SPEED) {
      ...
      if (actualMotorDir[1]) {
        SET(PORTD,6);//(PORTB,0);   // DIR Motor 2
      } else {
        CLR(PORTD,6);//(PORTB,0);
      }  
      subPeriodIndex[1] = (subPeriodIndex[1]+1)&0x07;
      
      SET(PORTD,3);//(PORTD,6); // STEP Motor 2
      delayMicroseconds(1);
      CLR(PORTD,3);//(PORTD,6);
    }
  }
...
void initOutput() {
  ...
  digitalWrite(8,LOW);//(4, LOW);   // Enable stepper drivers
